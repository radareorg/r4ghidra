 ...
 *
 * <p>This class provides functionality for evaluating mathematical expressions with support for: -
 * Different number bases (decimal, hexadecimal, octal, binary) - Symbol resolution through
 * callbacks - Memory access with bracketed expressions - Arithmetic operations - Parenthesized
 * expressions
 */
public class R2Num {
...
private List<Token> infixToPostfix(List<Token> infix) throws R2NumException {
	...
	switch (token.type) {
		...
		while (!operatorStack.isEmpty()
			...
			&& getPrecedence(operatorStack.peek().value.charAt(0))
				>= getPrecedence(token.value.charAt(0))) {
			postfix.add(operatorStack.pop());
		}
...
private int getPrecedence(char op) {
	switch (op) {
	case '~':
		return 4; // Bitwise NOT (highest precedence)
	case '*':
	case '/':
	case '%':
	...
	case '|':
	case '^':
		return 0; // Bitwise operations (lowest precedence)
	default:
		return -1;
	}
}
...
private long evaluateBracketExpression(String bracketExpr) throws R2NumException {
	if (bracketExpr == null
		|| bracketExpr.length() < 2
		|| !bracketExpr.startsWith("[")
		|| !bracketExpr.endsWith("]")) {
