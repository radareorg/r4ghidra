 ...
 *
 * <p>This command provides information about the program and its architecture.
 */
...
private String executeBasicInfoCommand(R2Command command, R2Context context)
...
private String formatInfoR2(Program program, String arch, String bits, String processor) {
	...
	try {
	// Try to get actual entry point from executable path
	Address entryPoint = null;
	if (program.getExecutablePath() != null) {
		entryPoint = program.getImageBase();
	}

	// If no entry point found, use image base
	if (entryPoint == null) {
		entryPoint = program.getImageBase();
	}
	...
	sb.append("# language ")
		.append(program.getLanguage().getLanguageID().getIdAsString())
		.append("\n");
