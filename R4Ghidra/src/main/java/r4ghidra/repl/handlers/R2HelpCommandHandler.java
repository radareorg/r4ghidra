	...
	case "test":
		...
		try {
			...
			java.lang.reflect.Method testMethod =
				repl.getClass().getMethod("testCommentHandling");
...
private String evaluateExpressionInDecimal(String expr, R2Context context)
...
private String evaluateExpressionMultiBase(String expr, R2Context context)
	throws R2CommandException {
	try {
	long value = R2NumUtil.evaluateExpression(context, expr);
	StringBuilder sb = new StringBuilder();
...
private void processSubcommandsRecursively(
	...
	try {
		...
		processSubcommandsRecursively(
			output, handler, fullCommand, currentDepth + 1, maxDepth, filter);
	} catch (Exception e) {
		// Ignore errors for subcommands
	}
	}
...
public String getHelp() {
	...
	sb.append(
		" ?*~pattern    recursively show help for all commands, then filter lines matching pattern\n");
