...
public class R2CommentCommandHandler implements R2CommandHandler {
    ...
    /**
     * Execute the CCu command to set a unique comment at the current address
     * Format: CCu [comment] @ addr
     * Special format: CCu base64:[encoded] @ addr - decodes base64 content first
     */
    private String executeCCuCommand(R2Command command, R2Context context) throws R2CommandException {
        ...
        Address address = command.hasTemporaryAddress() ? 
                          command.getTemporaryAddress() : 
