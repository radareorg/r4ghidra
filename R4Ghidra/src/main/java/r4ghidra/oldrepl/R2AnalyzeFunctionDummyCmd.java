...
public boolean canHandle(char cmdChar) {
	if (cmdChar == '*' || cmdChar == 'i' || cmdChar == 'b') return true;
	return false;
}
...
private String cmdAfb(String cmd) {
	...
	afS 0 @ 0x0
	*/
	FlatProgramAPI ghidra = R4GhidraState.api;
	Address currentAddress = R4GhidraState.r2Seek;
	try {
	CodeBlockModel cbm =
		new BasicBlockModel(
			ghidra.getCurrentProgram()); // blockModelService.getActiveBlockModel();
...
private String handleI(String cmd) {
	if (cmd.length() > cmdPos + 1 && cmd.charAt(cmdPos + 1) == '*') return cmdAfb(cmd);
	try {
	FlatProgramAPI ghidra = R4GhidraState.api;
	Address currentAddress = R4GhidraState.r2Seek;
...
public String handle(String cmd) {
	if (cmd.charAt(cmdPos) == 'i') return handleI(cmd);
	return cmdAfb(cmd);
}
}
